...
try {
    ...
    & node $runtimeHelper stop electron "ZipKit" "ZipKit"
    if ($LASTEXITCODE -ne 0) { throw "Could not replace the existing ZipKit runtime." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "ZipKit did not start." }
}
catch {
    Write-Host ""
