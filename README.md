...
| `--timestamps <preserve\|clamp>` | `preserve` | Timestamp policy. `preserve` (default) writes the DOS local-time field *and* the Info-ZIP extended-timestamp (`0x5455`) and NTFS (`0x000a`) extras, each carrying modification, access, and creation times as absolute UTC — so a reader on any platform recovers the right instant, and Windows restores full 100-ns precision. Unknown extras are skipped by every conforming reader, so this is safe for old tools. `clamp` writes only the DOS local-time field (2-second resolution, clamped to 1980–2107) for a minimal, zero-extra archive. |
